...
M.deprecated_extras = {
  ["ohmynvim.plugins.extras.formatting.conform"] = "`conform.nvim` is now the default **OhMyNvim** formatter.",
  ["ohmynvim.plugins.extras.linting.nvim-lint"] = "`nvim-lint` is now the default **OhMyNvim** linter.",
}
...
function M.extra_idx(name)
  ...
  for i, extra in ipairs(Config.spec.modules) do
    if extra == "ohmynvim.plugins.extras." .. name then
      return i
    end
  end
end
...
function M.fix_imports()
  Plugin.Spec.import = Util.inject.args(Plugin.Spec.import, function(_, spec)
    ...
    if dep then
      ...
      Util.warn(dep, { title = "OhMyNvim", once = true, stacktrace = true, stacklevel = 6 })
      return false
    end
  end)
end

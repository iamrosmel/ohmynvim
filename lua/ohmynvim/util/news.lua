...
function M.welcome()
  Util.info("Welcome to OhMyNvim!")
end

function M.changelog()
  M.open("CHANGELOG.md", { plugin = "ohmynvim" })
end

function M.ohmynvim(when_changed)
  M.open("NEWS.md", { plugin = "ohmynvim", when_changed = when_changed })
end

...
function M.migrate()
  Util.info("Migrating `ohmynvim.json` to version `" .. Config.json.version .. "`")
  ...
  if not json.data.version then
    ...
    json.data.extras = vim.tbl_map(function(extra)
      return "ohmynvim.plugins.extras." .. extra
    end, json.data.extras or {})
  elseif json.data.version == 1 then
    json.data.extras = vim.tbl_map(function(extra)
      -- replace double extras module name
      return extra:gsub("^ohmynvim%.plugins%.extras%.ohmynvim%.plugins%.extras%.", "ohmynvim.plugins.extras.")
    end, json.data.extras or {})
  end

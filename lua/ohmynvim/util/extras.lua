local Config = require("ohmynvim.config")
...
---@class ohmynvim.util.extras
local M = {}

---@type LazyExtraSource[]
M.sources = {
  { name = "OhMyNvim", desc = "OhMyNvim extras", module = "ohmynvim.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("ohmynvim.extras")
---@type string[]
M.state = nil
...
function X.new()
  ...
  self.float = Float.new({ title = "OhMyNvim Extras" })
    ...
    if extra.row == pos[1] then
      ...
      Util.info(
        "`"
          ...
          .. "\nPlease restart OhMyNvim to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("OhMyNvim Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled OhMyNvim extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  ...
  if extra.source.name ~= "ohmynvim" then
